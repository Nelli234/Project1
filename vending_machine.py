...
#TPRG2131
#11/16/2023
#Updated by Nelson Hucklebridge (100614680) for project 1
...
#Detect if pi is being used
hardware_present = False
try:
    from gpiozero import Button, Servo#if the pi is used import button and servo
    servo = Servo(17)#GPIO 17
    key1 = Button(5)#GPIO 5
    hardware_present = True#set hardware too present
except ModuleNotFoundError:
    print("Not on a Raspberry Pi or gpiozero not installed.")#lets user know no hardware is set up

# Setting this constant to True enables the logging function
# Set it to False for normal operation
TESTING = True

# Print a debug log string if TESTING is True.
def log(s):
    if TESTING:
        print(s)


# Vending state machine class holds the states and any information
# that "belongs to" the state machine
class VendingMachine(object):
    #products avaible and their worth
...
class DeliverProductState(State):
    ...
    def on_entry(self, machine):
        ...
        machine.amount = 0
        # simulation for product dropping
